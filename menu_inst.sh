...
# Configuración
PROJECT_DIR="/etc/argentina"
KEY_DATABASE="$PROJECT_DIR/licencias.db"
SECRET_MASTER_KEY="CambiarPorTuClaveSecreta123!"
MAX_ATTEMPTS=3

# Paleta de Colores Neon
cyan="\033[1;36m"      # Cyan
purple="\033[1;95m"    # Morado
yellow="\033[1;93m"    # Amarillo
green="\033[1;92m"     # Verde
red="\033[1;91m"       # Rojo
reset="\033[0m"        # Reset
#━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# VARIABLES DEL SISTEMA (AGREGAR ESTO)
#━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ADM_inst="$PROJECT_DIR/herramientas"    # Ruta de scripts de instalación
ADM_tmp="$PROJECT_DIR/tmp"             # Directorio temporal
ADM_crt="$PROJECT_DIR/crt"             # Certificados SSL
ADMRufu="$PROJECT_DIR"                 # Ruta principal (compatibilidad)

# Función de verificación de licencia
check_license() {
  clear
  echo -e "${purple}
╔════════════════════════════════════════╗
...
╚════════════════════════════════════════╝${reset}"
  ...
  for ((i=1; i<=$MAX_ATTEMPTS; i++)); do
    echo -ne "${cyan}[?] Ingrese su clave de licencia (${i}/${MAX_ATTEMPTS}): ${reset}"
    read -s user_key
    echo  # Salto de línea después de entrada oculta

    entry=$(grep "^${user_key}:" "$KEY_DATABASE")
    [ -z "$entry" ] && continue

    stored_hash=$(echo "$entry" | cut -d: -f2)
    current_uses=$(echo "$entry" | cut -d: -f3)
    generated_hash=$(echo "${user_key}${SECRET_MASTER_KEY}" | sha256sum | head -c 16)

    if [[ "$generated_hash" == "$stored_hash" && $current_uses -lt 3 ]]; then
      sed -i "s/^${entry}$/$(echo "$entry" | awk -F: -v uses=$(($current_uses +1)) '{$3=uses}1' OFS=':')/" "$KEY_DATABASE"
      return 0
    fi
  done
...
# Resto del menú principal (tus funciones aquí)
# ... (incluir las funciones originales del menu_inst.sh aquí)
...
fun_autorun () {
  clear
  msg -bar
if [[ $(cat /etc/bash.bashrc | grep -w ${Argentina}/menu) ]]; then
  cat /etc/bash.bashrc | grep -v ${Argentina}/menu > /tmp/bash
  mv -f /tmp/bash /etc/bash.bashrc
  msg -ama "               $(fun_trans "AUTO-INICIO REMOVIDO")"
  msg -bar
else
  cp /etc/bash.bashrc /tmp/bash
  echo "${Argentina}/menu" >> /tmp/bash
  mv -f /tmp/bash /etc/bash.bashrc
